<?php
/**
 * changeme functions and definitions
 *
...
/* Add Menus */
add_action('init', 'changeme_custom_menu');

/* Dashboard Config */
add_action('wp_dashboard_setup', 'changeme_dashboard_widget');

/* Dashboard Style */
add_action('admin_head', 'changeme_custom_fonts');

/* Remove Default Menu Items */
add_action('admin_menu', 'changeme_remove_menus');

/* Change Posts Columns */
add_filter('manage_posts_columns', 'changeme_manage_columns');

/* Reorder Admin Menu */
add_filter('custom_menu_order', 'changeme_reorder_menu');
add_filter('menu_order', 'changeme_reorder_menu');

/* Remove Comments Link */
add_action('wp_before_admin_bar_render', 'changeme_manage_admin_bar');
...
function changeme_scripts() {
	wp_enqueue_style( 'changeme-style', get_template_directory_uri() . '/style.css', array(), filemtime(get_template_directory() . '/style.css'), false);
	wp_enqueue_script( 'changeme-core-js', get_template_directory_uri() . '/inc/js/compiled.js', array('jquery'), filemtime( get_stylesheet_directory() . '/inc/js/compiled.js' ), true);
}

add_filter( 'script_loader_tag', function ( $tag, $handle ) {

	if ( 'changeme-intersection-js' !== $handle )
		return $tag;

	return str_replace( ' src', ' defer="defer" src', $tag );
}, 10, 2 );

function changeme_custom_menu() {
...
function changeme_dashboard_widget() {
	global $wp_meta_boxes;
	wp_add_dashboard_widget('custom_help_widget', 'changeme Support', 'changeme_dashboard_help');
}

function changeme_dashboard_help() {
	echo file_get_contents(__DIR__ . "/admin-settings/dashboard.html");
}

function changeme_custom_fonts() {
...
function changeme_remove_menus(){
	remove_menu_page( 'edit-comments.php' ); //Comments
}

function changeme_manage_columns($columns) {
	unset($columns["comments"]);
	return $columns;
}

function changeme_reorder_menu() {
    return array(
		'index.php',                        // Dashboard
		'separator1',                       // --Space--
		'edit.php',                         // Posts
...
function changeme_manage_admin_bar(){
	global $wp_admin_bar;
